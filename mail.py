...
def check(user):
    ...
    targetDayTable = pd.read_csv(path)[['time',test.columns[targetweek]]]
...
def emailTable(receiversMail,targetDayTablePath):
    ...
    #密码(部分邮箱为授权码) 
    mail_pass = ''   #使用授权码这个的获得需要从邮箱设置里面找~
    #邮件发送方邮箱地址
    ...
    if len(nightClass) ==0:
        nightText="上午没课嗷，可以快乐玩耍！~"
        pass
    else:
        nightClassStart = nightTable[nightTable.iloc[:,1] == nightClass[0]]['time'].iloc[0].split('~')[0]
        nightClassEnd = nightTable[noonTable.iloc[:,1] == nightClass[0]]['time'].iloc[-1].split('~')[1]
    ...
    part_attach1.add_header('Content-Disposition','attachment',filename="tommorw.csv") #为附件命名
